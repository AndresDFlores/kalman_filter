...
class Kalman1D(KalmanFilter):
    ...
    def __init__(self, literal=False):
        ...
        self.measured = {key:[] for key in keys}  # replace lists with dequeue to keep this from growing indefinitely
    ...
    def main(self):
        ...
        #  DEV - delete
        self.kf_results['x_position']['idx'].append(self.iter)
        self.kf_results['x_position']['predicted'].append(x_curr_pred[0, 0])
        self.kf_results['x_position']['measured'].append(self.current_state[0, 0])
        self.kf_results['x_position']['kalman'].append(x_corrected[0, 0])

        self.kf_results['x_velocity']['idx'].append(self.iter)
        self.kf_results['x_velocity']['predicted'].append(x_curr_pred[1, 0])
        self.kf_results['x_velocity']['measured'].append(self.current_state[1, 0])
        self.kf_results['x_velocity']['kalman'].append(x_corrected[1, 0])
